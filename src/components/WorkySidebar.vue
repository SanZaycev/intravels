<template>
<div class="w-sidebar col-12 col-lg-3" :class="fetchingClass">
  <aside id="sidebar" class="right-sidebar">
    <div class="sidebar-inner">
      <div class="sidebar-widget">
        <a :href="bannerPR" class="pr__url"><img :src="bannerURI" alt="Banner Sidebar" class="pr__img"></a>
      </div>
    </div>
  </aside>
</div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("location", ["bannerURI", "bannerPR"]),
    fetchingClass() {
      return this.isListFetching ? "fetching" : ""
    }
  },
  methods: {
    initialize(){
    ...
    go(){
      if (!this.active && document.documentElement.clientWidth >= 992){
        this._listener();
    ...
    _onWindowResize(){
      if (document.documentElement.clientWidth < 992){ this.destroy(); }
      else {
